...
class Tokenizer(object):
    ...
    def tokenize(self, text, *args, **kwrags):
        for token in self._filter(self._pre_normalizer(self._split(text))):
...
class AlphaTokenizer(Tokenizer):
    ...
    """
    def __init__(self, keep_digits=False, filter=None, pre_normalizer=None, post_normalizer=None, lower_case_text=False):
        ...
        self._text_to_lower = lower_case_text

    def _split(self, text):
        return self._alpha_filter.filter(text.lower() if self._text_to_lower else text).split()
